...
//! Class constructor
TabulatedGravityFieldVariationsWithCosineFunction::TabulatedGravityFieldVariationsWithCosineFunction(
        std::function< std::map< double, Eigen::MatrixXd > () > cosineCoefficientCorrectionsFunction,
        const std::map< double, Eigen::MatrixXd >& sineCoefficientCorrections,
        const int minimumDegree, const int minimumOrder,
        const std::shared_ptr< interpolators::InterpolatorSettings >interpolatorType ):
    GravityFieldVariations( minimumDegree, minimumOrder,
                            minimumDegree + cosineCoefficientCorrectionsFunction( ).begin( )->second.rows( ) - 1,
                            minimumOrder + cosineCoefficientCorrectionsFunction( ).begin( )->second.cols( ) - 1 ),
    interpolatorType_( interpolatorType )
{
    cosineCoefficientCorrectionsFunction_ = cosineCoefficientCorrectionsFunction;

    // Create interpolator for tabulated coefficients.
    resetCoefficientInterpolator( cosineCoefficientCorrectionsFunction, sineCoefficientCorrections );
}

//! Function to (re)set the tabulated spherical harmonic coefficients.
void TabulatedGravityFieldVariationsWithCosineFunction::resetCoefficientInterpolator(
        std::function< std::map< double, Eigen::MatrixXd > () > cosineCoefficientCorrectionsFunction,
        const std::map< double, Eigen::MatrixXd > sineCoefficientCorrections )
{
    // Set current coefficient tables.
    cosineCoefficientCorrections_ = cosineCoefficientCorrectionsFunction( );
    sineCoefficientCorrections_ = sineCoefficientCorrections;
...
//! Function for calculating corrections by interpolating tabulated corrections.
std::pair< Eigen::MatrixXd, Eigen::MatrixXd > TabulatedGravityFieldVariationsWithCosineFunction::
calculateSphericalHarmonicsCorrections(
        const double time )
{
    // Interpolate corrections
