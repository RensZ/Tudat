        ...
        std::shared_ptr< estimatable_parameters::EstimatableParameterSet< InitialStateParameterType > >( ) )
{
    ...
    case time_varying_gravitational_parameter_acceleration:
        ...
        // Check if identifier is consistent with type.
        if( std::dynamic_pointer_cast< TimeVaryingGravitationalParameterAcceleration >( accelerationModel ) == nullptr )
        {
            throw std::runtime_error( "Acceleration class type does not match acceleration type "
        ...
        {
            // Create partial-calculating object.
            accelerationPartial = std::make_shared< TimeVaryingGravitationalParameterPartial  >
                    ( std::dynamic_pointer_cast< TimeVaryingGravitationalParameterAcceleration >( accelerationModel ),
                      acceleratedBody.first, acceleratingBody.first );
        }
        break;
